...
def _validate_units(
    ...
    *,
    ...
    checks: list[str] = [
        ...
        "systemctl --user is-enabled v5-quant-lab-selfcheck.timer",
        ...
        "systemctl --user is-enabled v5-daily-trend-paper.timer",
        "systemctl --user is-enabled v5-paired-reference-paper.timer",
        "systemctl --user is-enabled v5-live-cost-evidence.timer",
        "test \"$(systemctl --user is-enabled v5-daily-ml-training.timer)\" = disabled",
        "test \"$(systemctl --user is-enabled v5-model-promotion-gate.timer)\" = disabled",
        "test \"$(systemctl --user is-enabled v5-shadow-tuned-xgboost.user.timer)\" = disabled",
        "test \"$(systemctl --user is-enabled v5-review-forward.timer)\" != enabled",
        "test \"$(systemctl --user is-enabled v5-participation-quotes.service)\" != enabled",
        "test \"$(systemctl --user is-active v5-web-dashboard.service)\" = active",
        "test \"$(systemctl --user is-active v5-trade-monitor.timer)\" = active",
        "test \"$(systemctl --user is-active v5-quant-lab-selfcheck.timer)\" = active",
        ...
        "test \"$(systemctl --user is-active v5-daily-trend-paper.timer)\" = active",
        "test \"$(systemctl --user is-active v5-paired-reference-paper.timer)\" = active",
        "test \"$(systemctl --user is-active v5-live-cost-evidence.timer)\" = active",
        ...
        "test \"$(systemctl --user is-active v5-review-forward.timer)\" != active",
        "test \"$(systemctl --user is-active v5-participation-quotes.service)\" != active",
        ...
        "systemctl --user show v5-quant-lab-selfcheck.timer --property=UnitFileState,ActiveState",
        ...
        "systemctl --user show v5-daily-trend-paper.timer --property=UnitFileState,ActiveState",
        "systemctl --user show v5-participation-quotes.service --property=UnitFileState,ActiveState",
    ]
    if enable_prod_timer:
        checks.extend(
            [
