...
def fetch_candles_simple(symbol, before_time, limit=100):
    """获取K线数据 - 简化版本，只取需要的字段"""
        ...
        if data.get('code') == '0':
            ...
            for candle in data.get('data', []):
                ...
                candles.append({
                    ...
                    # 忽略 candle[6] (volume_ccy), candle[7], candle[8]
                })
            return candles
        else:
...
def backfill_symbol_final(symbol, days=30):
    """最终回填函数"""
    ...
    print(f"  目标时间范围: {datetime.fromtimestamp(start_time)} 到 {datetime.fromtimestamp(end_time)}")
    
    # 获取当前数据的最早时间
    cursor.execute("SELECT MIN(timestamp) FROM market_data_1h WHERE symbol = ?", (symbol,))
    earliest_existing = cursor.fetchone()[0]
    
    if earliest_existing:
        print(f"  当前最早数据: {datetime.fromtimestamp(earliest_existing)}")
        # 如果已有数据，从最早数据时间向前获取
        fetch_before = earliest_existing - 3600
    else:
        print(f"  当前没有数据")
        fetch_before = end_time
    
    added = 0
    current_before = fetch_before
    batch_count = 0
    
    # 分批获取数据
    while current_before > start_time and batch_count < 20:
        batch_count += 1
        
        candles = fetch_candles_simple(symbol, current_before, limit=100)
        ...
        print(f"   批次{batch_count}: 获取{len(candles)}条，有效{len(valid_candles)}条，新增{added}条")
...
def main():
    ...
    for i, symbol in enumerate(test_symbols, 1):
        ...
        try:
            added = backfill_symbol_final(symbol, days=30)
            if added > 0:
                total_added += added
    ...
    with open('reports/final_backfill_report.json', 'w') as f:
        json.dump(report, f, indent=2)
    
    print(f"\n📋 报告已保存: reports/final_backfill_report.json")
