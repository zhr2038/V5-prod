...
class QualityCheckPaths:
    workspace: Path
    reports_dir: Path
    alpha_history_db: Path
...
def build_paths(workspace: Path | None = None) -> QualityCheckPaths:
    ...
    return QualityCheckPaths(
        ...
        alpha_history_db=reports_dir / "alpha_history.db",
    )
...
def _open_alpha_history_db(paths: QualityCheckPaths) -> sqlite3.Connection:
    if not paths.alpha_history_db.exists():
        raise FileNotFoundError(f"未找到数据质量数据库: {paths.alpha_history_db}")
    return sqlite3.connect(str(paths.alpha_history_db))


def run_data_quality_checks(paths: QualityCheckPaths | None = None) -> dict[str, int]:
    """运行数据质量检查。"""
    resolved_paths = paths or build_paths()

    print("运行数据质量检查")
    print("=" * 60)
    print(f"数据库: {resolved_paths.alpha_history_db}")

    conn = _open_alpha_history_db(resolved_paths)
    cursor = conn.cursor()
    ...
    try:
        ...
        print("\n质量分布:")
        print(f"  优秀: {excellent}个 ({_pct(excellent, len(results)):.1f}%)")
        print(f"  良好: {good}个 ({_pct(good, len(results)):.1f}%)")
        print(f"  一般: {average}个 ({_pct(average, len(results)):.1f}%)")
        print(f"  需改进: {need_improvement}个 ({_pct(need_improvement, len(results)):.1f}%)")
        ...
        perfect_count = sum(1 for row in quality_results if row[5] == "优秀")
        issues_count = len(quality_results) - perfect_count

        print(f"完美币种: {perfect_count}个 ({_pct(perfect_count, len(quality_results)):.1f}%)")
        print(f"存在问题: {issues_count}个 ({_pct(issues_count, len(quality_results)):.1f}%)")
        ...
        continuous = sum(1 for row in continuity_results if row[5] == "连续")
        small_gaps = sum(1 for row in continuity_results if row[5] == "少量缺口")
        need_optimization = sum(1 for row in continuity_results if row[5] == "需优化")

        print(f"完全连续: {continuous}个 ({_pct(continuous, len(continuity_results)):.1f}%)")
        print(f"少量缺口: {small_gaps}个 ({_pct(small_gaps, len(continuity_results)):.1f}%)")
        print(f"需优化: {need_optimization}个 ({_pct(need_optimization, len(continuity_results)):.1f}%)")
...
def create_data_optimization_plan(stats: dict[str, int]) -> bool:
    """创建数据优化计划。"""
    print("\n4. 数据优化计划")
    print("=" * 60)
    print("优化优先级:")
...
def main() -> int:
    ...
    print(
        f"- 数据质量: {stats['excellent_quality']}优秀, {stats['good_quality']}良好, "
        f"{stats['average_quality']}一般, {stats['need_improvement']}需改进"
    )
    print(
        f"- 数据准确性: {stats['perfect_data_quality']}完美, "
        f"{stats['data_issues']}存在问题"
    )
    print(
        f"- 时间连续性: {stats['continuous']}连续, "
        f"{stats['small_gaps']}少量缺口, {stats['need_optimization']}需优化"
    )
