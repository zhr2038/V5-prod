...
def main():
    ...
    with (root / "worker.lock").open("a") as lock:
        ...
        try:
            ...
            experiment = json.loads((project / "configs/research/review_experiment_v1.json").read_text(encoding="utf-8"))
            ...
            _save_report(root / "worker-status.json", {"ok": False, "error": type(exc).__name__, "detail": str(exc)[:1000], "live_order_effect": "none"})
            raise
