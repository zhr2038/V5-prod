...
import { normalizeChartSymbol } from '../lib/marketChart';
import ParticipationExecution from './ParticipationExecution';
import DailyTrendPaper from './DailyTrendPaper';
import PairedReferencePaper from './PairedReferencePaper';
import LiveCostEvidence from './LiveCostEvidence';
...
export function CommandCenter(props: Props) {
  ...
  const chartCandidate = command?.candidates.find(candidate => chartSymbol && normalizeChartSymbol(candidate.symbol) === normalizeChartSymbol(chartSymbol));
  const chartPosition = knownPositions && !primaryBad ? positions?.find(position => chartSymbol && position.qty > 0 && normalizeChartSymbol(position.symbol) === normalizeChartSymbol(chartSymbol)) : null;
    ...
    <div className="cc-main">
      ...
      <main className="cc-content">
        ...
        <section id="opportunities" className="cc-section">
          ...
          {chartSymbol && <div className="cc-market-detail"><div className="cc-detail-title"><h3>{baseSymbol(chartSymbol)} 市场详情</h3><button className="cc-icon-button" onClick={() => setChartSymbol(null)} aria-label="收起市场详情"><X size={17} /></button></div>{chartCandidate && <div className="cc-candidate-detail"><dl><div><dt>决策参考价</dt><dd>{number(chartCandidate.reference_price)} USDT</dd></div><div><dt>Alpha 排序分</dt><dd>{number(chartCandidate.alpha_score, 4)}</dd></div><div><dt>目标仓位</dt><dd>{ratio(chartCandidate.target_weight)}</dd></div><div><dt>报价时点</dt><dd>{dateTime(chartCandidate.price_as_of)}</dd></div></dl><p>{[...new Set([...(chartCandidate.selection_reasons || []), ...(chartCandidate.router_reasons || [])])].map(reason => <span key={reason} title={reason}>{reasonLabel(reason)}</span>)}</p></div>}<Suspense fallback={<LoadingNote />}><ExchangeChart focusSymbol={chartSymbol} costPrice={chartPosition?.avgPrice} onSymbolChange={symbol => { setChartSymbol(symbol); onSymbolSearch(symbol); }} /></Suspense></div>}
        ...
        <section id="participation" className="cc-section"><SectionTitle index="04" title="模拟账户与新方向" detail="日线趋势验证与交易参考同资金对照；旧实验保留为历史证据。" aside={<span className="cc-small-badge cc-paper-badge"><FlaskConical size={13} /> 模拟资金</span>} />
          ...
          {participation && <ParticipationExecution data={participation} now={now} />}
          <div className="cc-review-footnote"><p>实盘最近决策记录：<code>{run?.run_id || '待确认'}</code> · 实盘账户及成交见“持仓与成交”。</p><p>原 participation 账本身份：<code style={{ overflowWrap: 'anywhere' }}>{participation?.identity || '待确认'}</code></p><p>原策略哈希：<code style={{ overflowWrap: 'anywhere' }}>{participation?.policy_hash || '待确认'}</code> · 起点 {dateTime(participation?.cohort_started_at)}</p></div>
          <DailyTrendPaper data={command?.daily_trend_paper} unavailable={commandStale} />
          <PairedReferencePaper data={command?.paired_reference_paper} unavailable={commandStale} />
          <LiveCostEvidence data={command?.live_cost_evidence} unavailable={commandStale} />
          {!participation?.quote_execution_enabled && participation?.latest_decision?.candidate_reasons && <details className="cc-disclosure"><summary>查看新策略逐币决策依据<ChevronDown size={14} /></summary><div className="cc-paper-reasons">{Object.entries(participation.latest_decision.candidate_reasons).map(([symbol, reason]) => <div key={symbol}><b>{symbol}</b><span>{reasonLabel(reason)}</span><code>{reason}</code></div>)}</div></details>}
        </section>
