...
class V5Pipeline:
    ...
    @staticmethod
    def _signal_factor_value(signal: Optional[Dict[str, Any]], factor_name: str) -> Optional[float]:
        if not isinstance(signal, dict):
            return None
        ...
        for bucket in ("z_factors", "raw_factors"):
            ...
            if factor_name not in values:
                continue
            try:
                return float(values.get(factor_name))
            except Exception:
                continue
        return None
    ...
    def _alpha6_rsi_confirm(signal: Optional[Dict[str, Any]]) -> Optional[float]:
        return V5Pipeline._signal_factor_value(signal, "f5_rsi_trend_confirm")

    @staticmethod
    def _alpha6_volume_expansion(signal: Optional[Dict[str, Any]]) -> Optional[float]:
        return V5Pipeline._signal_factor_value(signal, "f4_volume_expansion")
    ...
    def run(
        ...
        exchange_rules_by_symbol: Dict[str, Any] = {}

        def _exchange_rules_for_symbol(sym: str, *, suppress_errors: bool = True) -> Any:
            key = str(sym or "")
            if key in exchange_rules_by_symbol:
                return exchange_rules_by_symbol[key]
            try:
                spec = instrument_cache.get_spec(key.replace("/", "-"))
            except Exception:
                if not suppress_errors:
                    raise
                spec = None
            exchange_rules_by_symbol[key] = spec
            return spec

        def _dust_meta_for_position(sym: str, qty: float, value_usdt: float) -> Tuple[bool, Dict[str, Any]]:
            exchange_rules = _exchange_rules_for_symbol(sym, suppress_errors=True)
            threshold = dust_position_threshold_usdt(sym, qty, value_usdt, self.cfg, exchange_rules)
            is_dust = is_dust_position(sym, qty, value_usdt, self.cfg, exchange_rules)
            return is_dust, {
                "raw_held_value_usdt": float(abs(float(value_usdt or 0.0))),
                "effective_held_value_usdt": 0.0 if is_dust else float(abs(float(value_usdt or 0.0))),
                "dust_threshold_usdt": float(threshold),
            }

        filtered_selected = []
        skipped_for_minsz = []
        for sym in (portfolio.selected or []):
            spec = _exchange_rules_for_symbol(sym, suppress_errors=False)
        ...
        def _has_effective_non_dust_position() -> bool:
            for pos in positions:
                ...
                is_dust_pos, _ = _dust_meta_for_position(pos.symbol, qty_pos, position_value)
                if not is_dust_pos:
                    return True
            return False
        ...
        if btc_probe_enabled:
            ...
            if self._live_symbol_whitelist and btc_symbol not in self._live_symbol_whitelist:
                ...
                if active_neg_cooldown:
                    ...
                    required_notional = float(getattr(self.cfg.budget, "min_trade_notional_base", 0.0) or 0.0)
                    spec = _exchange_rules_for_symbol(btc_symbol, suppress_errors=True)
                    try:
                        min_sz = float(getattr(spec, "min_sz", 0.0) or 0.0) if spec is not None else 0.0
                        slack = float(getattr(self.cfg.budget, "exchange_min_notional_slack_multiplier", 1.05) or 1.05)
                        if min_sz > 0 and last_px > 0:
                            required_notional = max(required_notional, min_sz * last_px * slack)
                    except Exception:
                        pass
        ...
        # Merge all exit orders: probe first, then profit, stop, rank, then policy
        exit_orders = probe_exit_orders + profit_orders + fixed_stop_orders + ranking_exit_orders + exit_orders

        # Deduplicate: keep only one exit per symbol per round, priority: sell_all > dynamic_stop > fixed_stop > atr > partial
        if exit_orders:
            prio_map = {
                'probe_take_profit': 120,
                'probe_stop_loss': 120,
                'probe_trailing_stop': 120,
                'probe_time_stop': 120,
        ...
        dust_close_router_decisions = []
        if exit_orders:
            filtered_exit_orders = []
            positions_by_symbol = {str(getattr(p, "symbol", "")): p for p in (positions or [])}
            for order in exit_orders:
                p = positions_by_symbol.get(str(order.symbol))
                if p is not None and str(order.side).lower() == "sell":
                    px_exit = float(order.signal_price or prices.get(order.symbol, 0.0) or 0.0)
                    qty_exit = float(getattr(p, "qty", 0.0) or 0.0)
                    raw_value_exit = qty_exit * px_exit if px_exit > 0 else float(order.notional_usdt or 0.0)
                    is_dust_exit, dust_meta = _dust_meta_for_position(order.symbol, qty_exit, raw_value_exit)
                    if is_dust_exit:
                        if audit:
                            audit.record_count("dust_residual_no_close_order_count", symbol=order.symbol)
                            audit.add_note(
                                f"Dust residual close suppressed: {order.symbol} "
                                f"value=${dust_meta['raw_held_value_usdt']:.6f} "
                                f"< threshold=${dust_meta['dust_threshold_usdt']:.6f}"
                            )
                            dust_close_router_decisions.append(
                                {
                                    "symbol": order.symbol,
                                    "action": "skip",
        ...
        exit_router_decisions = list(dust_close_router_decisions)
        for order in exit_orders:
            meta = dict(order.meta or {})
        ...
        if equity > 0:
            for p in positions:
                ...
                is_dust, dust_meta = _dust_meta_for_position(p.symbol, qty, position_value)
                if is_dust:
                    if audit and qty > 0:
                        audit.add_note(
                            f"Dust filter: {p.symbol} qty={qty:.8f} "
                            f"value=${dust_meta['raw_held_value_usdt']:.6f} "
                            f"(threshold=${dust_meta['dust_threshold_usdt']:.6f}) treated as 0"
                        )
                    continue
        ...
        for sym in symbols_all:
            ...
            if force_close_unscored and held is not None and scored_symbols and sym not in scored_symbols:
                ...
                if px_fs > 0:
                    ...
                    is_dust_close, dust_close_meta = _dust_meta_for_position(
                        sym,
                        float(getattr(held, 'qty', 0.0) or 0.0),
                        notional_fs,
                    )
                    if is_dust_close:
                        if audit:
                            ...
                            audit.add_note(
                                f"Dust residual force-close suppressed: {sym} "
                                f"value=${dust_close_meta['raw_held_value_usdt']:.6f} "
                                f"< threshold=${dust_close_meta['dust_threshold_usdt']:.6f}"
                            )
                            router_decisions.append(
                                {
                                    'symbol': sym,
            ...
            held_effective = held
            dust_add_size_meta = None
            if held is not None:
                raw_held_value = float(getattr(held, "qty", 0.0) or 0.0) * float(px)
                is_dust_held, dust_meta = _dust_meta_for_position(
                    sym,
                    float(getattr(held, "qty", 0.0) or 0.0),
                    raw_held_value,
                )
            ...
            if drift < 0:
                ...
                # drift > 0，需要加仓
                side = "buy"
                intent = "OPEN_LONG" if held_effective is None else "REBALANCE"
            ...
            bypass_turnover_cap_for_exit = bool(
                side == "sell" and held_effective is not None and abs(float(tw)) <= float(target_hold_eps)
            )

            if side == "buy" and dust_add_size_meta is not None and audit:
                audit.record_count("dust_position_ignored_for_add_size_count", symbol=sym)
                audit.add_note(
                    f"Dust position ignored for add-size: {sym} "
                    f"raw=${dust_add_size_meta['raw_held_value_usdt']:.6f} "
                    f"< threshold=${dust_add_size_meta['dust_threshold_usdt']:.6f}"
                )
                ...
                if protect_block is not None:
                    if audit:
                        if btc_probe_meta is not None:
                            audit.record_count(
                                "btc_leadership_probe_blocked_count",
                                symbol=f"{sym}:{str(protect_block.get('reason') or 'protect_entry')}",
                            )
                ...
                if blocked:
                    if audit:
                        if btc_probe_meta is not None:
                            audit.record_count(
                                "btc_leadership_probe_blocked_count",
                                symbol=f"{sym}:negative_expectancy_cooldown",
                            )
            ...
            if (
                ...
                if closed_cycles >= min_cycles and expectancy_bps < floor_bps:
                    if btc_probe_meta is not None and self._btc_leadership_probe_negative_bypass_allowed(neg_stats):
                        ...
                        if audit:
                            audit.record_count(
                                "btc_leadership_probe_negative_expectancy_bypass_count",
                                symbol=sym,
                            )
                            audit.add_note(
                                f"BTC leadership probe bypassed negative expectancy: "
                                f"{sym} cycles={closed_cycles}, net={expectancy_bps:.2f}bps"
                            )
                    else:
                        if audit:
                            if btc_probe_meta is not None:
                                audit.record_count(
                                    "btc_leadership_probe_blocked_count",
                                    symbol=f"{sym}:negative_expectancy_open_block",
                                )
                    ...
                    if btc_probe_meta is not None and self._btc_leadership_probe_negative_bypass_allowed(neg_stats):
                        ...
                        if audit:
                            if btc_probe_meta is not None:
                                audit.record_count(
                                    "btc_leadership_probe_blocked_count",
                                    symbol=f"{sym}:negative_expectancy_fast_fail_open_block",
                                )
            ...
            if side == "buy" and held_effective is not None and bool(getattr(self.cfg.execution, "anti_chase_enabled", False)):
                try:
                    entry_px = float(getattr(held_effective, "avg_px", 0.0) or 0.0)
                    held_qty = float(getattr(held_effective, "qty", 0.0) or 0.0)
                            ...
                            router_decisions.append(
                                {
                                    ...
                                    "raw_held_value_usdt": float(held_value),
            ...
            if side == "buy":
                ...
                if dust_add_size_meta is not None:
                    meta.update(dust_add_size_meta)
                if btc_probe_meta is not None:
            ...
            if audit and btc_probe_meta is not None:
            ...
            if audit:
                ...
                if side == "buy" and dust_add_size_meta is not None:
                    decision.update(dust_add_size_meta)
