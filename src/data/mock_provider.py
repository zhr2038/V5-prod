...
class MockProvider(MarketDataProvider):
    ...
    def fetch_ohlcv(self, symbols: List[str], timeframe: str, limit: int, end_ts_ms: int | None = None) -> Dict[str, MarketSeries]:
        ...
        for i, sym in enumerate(symbols):
            ...
            high = [max(o, c) * (1 + abs(float(self.rng.normal(0, 0.002)))) for o, c in zip(open_, close, strict=False)]
            low = [min(o, c) * (1 - abs(float(self.rng.normal(0, 0.002)))) for o, c in zip(open_, close, strict=False)]
