...
def test_reconcile_once_uses_runtime_positions_db_from_order_store_path(monkeypatch) -> None:
    captured = {}
    workspace = Path(reconcile_once.__file__).resolve().parents[1]
...
def test_reconcile_with_retry_uses_runtime_positions_db_from_order_store_path(monkeypatch) -> None:
    captured = {}
    workspace = Path(reconcile_with_retry.__file__).resolve().parents[1]
...
def test_reconcile_once_uses_runtime_reconcile_status_from_order_store_path_defaults(monkeypatch) -> None:
    captured = {}
    workspace = Path(reconcile_once.__file__).resolve().parents[1]
    ...
    assert captured["positions_db_path"] == (runtime_dir / "positions.sqlite").resolve()
    assert captured["account_db_path"] == (runtime_dir / "positions.sqlite").resolve()
    assert captured["out_path"] == (runtime_dir / "reconcile_status.json").resolve()


def test_reconcile_with_retry_uses_runtime_status_and_kill_switch_from_order_store_path_defaults(monkeypatch) -> None:
    captured = {}
    workspace = Path(reconcile_with_retry.__file__).resolve().parents[1]
    ...
    runtime_dir = (workspace / "reports" / "shadow_runtime").resolve()
    expected_kill = (runtime_dir / "kill_switch.json").resolve()
    assert captured["positions_db_path"] == (runtime_dir / "positions.sqlite").resolve()
    assert captured["account_db_path"] == (runtime_dir / "positions.sqlite").resolve()
    assert captured["out_path"] == (runtime_dir / "reconcile_status.json").resolve()
    assert captured["kill_switch_load_path"] == expected_kill
