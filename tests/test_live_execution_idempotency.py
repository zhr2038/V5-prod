...
def test_live_execution_rejects_external_rank_exit_without_router_validation_flag() -> None:
    with tempfile.TemporaryDirectory() as td:
        okx = FakeOKX()
        store = OrderStore(path=f"{td}/orders.sqlite")
        pos = PositionStore(path=f"{td}/pos.sqlite")
        ...
        result = eng.place(
            Order(
                ...
                signal_price=622.0,
                meta={
                    "decision_hash": "rank-exit-external-no-router-validation",
                    "reason": "rank_exit_4",
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        result = eng.place(
            Order(
                ...
                signal_price=600.0,
                meta={
                    "decision_hash": "bnb-atr-before-min-hold",
                    "reason": "atr_trailing",
                    "net_bps": -36.0,
        ...
        assert guard["net_bps"] == -36.0
        assert guard["f5_rsi_trend_confirm"] == 0.35
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        result = eng.place(
            Order(
                ...
                notional_usdt=12.0,
                signal_price=595.0,
                meta={"decision_hash": "bnb-atr-large-loss-before-min-hold", "reason": "atr_trailing", "net_bps": -100.0},
            )
        )
...
def test_fill_reconciler_records_take_profit_cooldown_for_already_filled_order() -> None:
    with tempfile.TemporaryDirectory() as td:
        ...
        store.update_state("TP2", new_state="FILLED", ord_id="2002")
        ...
        rec = FillReconciler(fill_store=fills, order_store=store, okx=None, position_store=None)
        rec.reconcile()
