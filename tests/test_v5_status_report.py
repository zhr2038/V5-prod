...
def test_get_current_risk_guard_prefers_latest_history_ts_when_history_is_unsorted(tmp_path: Path) -> None:
    reports_dir = tmp_path / "reports"
    reports_dir.mkdir(parents=True, exist_ok=True)
    cfg = {"execution": {"order_store_path": str((reports_dir / "shadow_orders.sqlite").resolve())}}
    (reports_dir / "shadow_auto_risk_guard.json").write_text(
        json.dumps(
            {
                ...
                "history": [
                    {"ts": "2026-04-19T14:05:00", "to": "DEFENSE"},
                    {"ts": "2026-04-19T13:00:00", "to": "PROTECT"},
                ],
            },
            ensure_ascii=False,
        ),
