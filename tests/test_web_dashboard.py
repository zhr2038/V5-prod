...
def test_quant_lab_cost_estimate_route_keeps_fresh_degraded_payload_non_stale(monkeypatch):
    module = load_web_dashboard_module()
    module._QUANT_LAB_PROXY_CACHE.clear()
    ...
    monkeypatch.setattr(module, "_utc_now", lambda: datetime(2026, 6, 25, 2, 45, tzinfo=timezone.utc))
    monkeypatch.setattr(
        module,
        "load_config",
        lambda: {
    ...
    with module.app.test_client() as client:
        response = client.get("/api/quant_lab/cost_estimate?symbol=BNB-USDT&regime=normal&notional_usdt=5&quantile=p75")
...
def test_health_api_keeps_quant_lab_cost_warning_advisory_when_live_coverage_pass(monkeypatch, tmp_path):
    module = load_web_dashboard_module()
    client = module.app.test_client()
    ...
    assert quant_lab["status"] == "healthy"
    assert quant_lab["request_metrics"]["latest_service_status"] == "warning"
...
def test_api_positions_prefers_fresh_local_price_cache_before_public_ticker(monkeypatch, tmp_path):
    ...
    cache_dir = tmp_path / "data" / "cache"
    cache_dir.mkdir(parents=True, exist_ok=True)
    cache_file = cache_dir / "ETH_USDT_1H_20260410_01.csv"
...
def test_api_positions_prefers_latest_row_in_unsorted_cache_file(monkeypatch, tmp_path):
    module = load_web_dashboard_module()
    monkeypatch.setattr(module, "REPORTS_DIR", tmp_path)
    monkeypatch.setattr(module, "WORKSPACE", tmp_path)
    monkeypatch.setattr(module, "_load_avg_cost_from_fills", lambda *args, **kwargs: None)
    monkeypatch.setattr(module.time, "time", lambda: datetime(2026, 4, 10, 1, 5).timestamp())
    ...
    cache_file.write_text(
        "\n".join(
            [
                ...
                "2026-04-10T01:00:00,100,101,99,222.22,10",
                "2026-04-10T00:00:00,100,101,99,111.11,10",
            ]
        ),
        encoding="utf-8",
    )
...
def test_okx_public_ticker_uses_params(monkeypatch):
    ...
    assert module._load_okx_public_ticker_last_price("ETH/USDT") == 234.56
    assert module._load_okx_public_ticker_last_price("OKX:ETH-USDT") == 234.56
    assert captured == [
        {
...
def test_okx_public_ticker_cache_uses_normalized_inst_id(monkeypatch):
    module = load_web_dashboard_module()
    module._OKX_PUBLIC_TICKER_CACHE.clear()
    monkeypatch.setenv("V5_DASHBOARD_PUBLIC_TICKER_CACHE_TTL_SECONDS", "60")
    calls = {"count": 0}
