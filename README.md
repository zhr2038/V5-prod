...
Premature swing soft exits are excluded from fast-fail hard blocks. These trades still remain visible as raw realized roundtrip losses, but if the close was a soft ATR/rank/regime/zero-target exit before the configured swing min-hold and diagnostics show the min-hold violation, it does not poison fast-fail negative expectancy.
